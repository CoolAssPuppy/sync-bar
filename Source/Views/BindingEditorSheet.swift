...
struct BindingEditorSheet: View {
    ...
    init(kind: DestinationKind, notebook: RmNotebook, existingBinding: DestinationBinding?, onSave: @escaping (DestinationBinding) -> Void, onCancel: @escaping () -> Void) {
        ...
        if let existingBinding {
            ...
            case .notion(let cfg):
                _localNotion = State(initialValue: NotionFormState(workspaceId: cfg.workspaceId, destinationId: cfg.destinationId, destinationType: cfg.destinationType, destinationTitle: cfg.destinationTitle))
            case .linear(let cfg):
